...
function publicHero() {
  return `<section class="public-hero">
    ...
    <div class="public-hero-copy public-hero__content">
      <p class="public-kicker">Islamabad, Pakistan</p>
      <h1>HAIDER ASSOCIATES <span>& BUILDERS</span></h1>
      <p>Real Estate, Architecture & Construction</p>
      <div class="hero-actions"><a class="btn btn-primary" href="#/properties">View Properties</a><a class="btn btn-on-dark" href="#/contact">Contact Haider Associates</a></div>
    </div>
  </section>`;
}
...
function home(state) {
  ...
  return `${publicHero()}
    ...
    <section class="public-section public-container featured-section">
      <div class="public-section-heading public-section__intro"><h2>Selected Islamabad properties</h2><p>Locally focused residential, commercial and rental opportunities managed through one operating team.</p></div>
    ...
    <section class="public-section public-container public-services-preview">
      ...
      <div class="service-copy"><h2>Property and construction, under one roof.</h2><p>From a sector search to a finished interior, Haider Associates coordinates the decisions that shape valuable property.</p>
    ...
    <section class="public-section public-container public-projects-preview"><div><h2>Built with a clear line of responsibility.</h2><p>Demo project records illustrate how design, construction, finance and documentation connect inside HAIDER OS.</p><a class="btn btn-navy" href="#/projects">View Projects</a></div><div class="project-image-grid">${projects.map((project, index) => `<article><img src="${escapeHTML(project.image || './assets/images/projects/villa-construction.jpg')}" width="960" height="640" loading="lazy" alt="${escapeHTML(project.name)}" /><h3>${escapeHTML(project.name)}</h3><p>${escapeHTML(project.location)} · ${escapeHTML(project.projectType)}</p></article>`).join('')}</div></section>
    <section class="public-contact-block"><div><p>Private client service</p><h2>Talk directly with Haider Associates.</h2></div><div><a href="tel:03009146600">0300-9146600</a><a class="btn btn-primary" href="#/contact">Contact Haider Associates</a></div></section>`;
}
...
function propertiesPage(state, query) {
  ...
  return `<section class="public-page-hero"><p class="public-kicker">Islamabad inventory</p><h1>Properties selected for serious buyers and tenants.</h1><p>Browse demo listings managed inside the HAIDER OS local workspace.</p></section>
...
function services() {
  return `<section class="public-page-hero"><p class="public-kicker">Integrated services</p><h1>One team from property decision to completed space.</h1><p>Practical coordination across Islamabad real estate, architecture and construction.</p></section><section class="public-section public-container service-index">${serviceItems.map(([name, body]) => `<article><div>${icon(name === 'Builders' ? 'hard-hat' : name === 'Rent' ? 'key-round' : 'ruler')}</div><h2>${name}</h2><p>${body}</p><a href="#/contact">Discuss this service ${icon('arrow-right')}</a></article>`).join('')}</section>`;
}

function about() {
  return `<section class="public-page-hero"><p class="public-kicker">Haider Associates & Builders</p><h1>Islamabad property knowledge with construction discipline.</h1><p>A recognizable local identity, translated into a more capable digital operating model.</p></section><section class="public-section public-container public-about-grid"><div><img src="./assets/images/properties/premium-interior.jpg" width="1536" height="1024" alt="Refined contemporary interior in an Islamabad home" /></div><div><h2>Built around accountable service.</h2><p>HAIDER OS is a demonstration of how the company can manage property inventory, clients, site visits, deals, commissions and projects without losing the personal service associated with Haider Cheema.</p><dl><div><dt>Primary contact</dt><dd>Haider Cheema</dd></div><div><dt>Location</dt><dd>Islamabad, Pakistan</dd></div><div><dt>Phone</dt><dd><a href="tel:03009146600">0300-9146600</a></dd></div></dl><p class="disclaimer">This product does not claim verified awards, certifications, reviews or completed project counts.</p></div></section>`;
}

function projectsPage(state) {
  return `<section class="public-page-hero"><p class="public-kicker">Architecture & construction</p><h1>Projects connected from brief to final handover.</h1><p>The records below are demonstration data and do not represent verified company projects.</p></section><section class="public-section public-container public-project-list">${state.entities.projects.map(project => `<article><img src="${escapeHTML(project.image || './assets/images/projects/villa-construction.jpg')}" width="1200" height="800" loading="lazy" alt="${escapeHTML(project.name)}" /><div><span>${escapeHTML(project.projectType)} · ${escapeHTML(project.status)}</span><h2>${escapeHTML(project.name)}</h2><p>${escapeHTML(project.location)} · ${escapeHTML(project.plotSize || '')}</p><div class="project-progress" aria-label="${escapeHTML(project.progress)} percent complete"><span style="--progress:${Number(project.progress) || 0}%"></span></div><strong>${escapeHTML(project.progress)}% complete</strong></div></article>`).join('')}</section>`;
}

function contact() {
  return `<section class="public-page-hero"><p class="public-kicker">Contact</p><h1>Start with the property or project decision in front of you.</h1><p>Send an enquiry or speak directly with Haider Associates in Islamabad.</p></section><section class="public-section public-container public-contact-layout"><div class="contact-details"><h2>Haider Associates & Builders</h2><p>Real Estate, Sale, Purchase, Rent, Architecture, Structure, Interior Design and Builders.</p><a href="tel:03009146600">${icon('phone')}<span><small>Call Haider Cheema</small>0300-9146600</span></a><a href="https://wa.me/923009146600" target="_blank" rel="noreferrer">${icon('message-circle')}<span><small>WhatsApp</small>Start a conversation</span></a><div>${icon('map-pin')}<span><small>Location</small>Islamabad, Pakistan</span></div></div><form class="public-contact-form" data-contact-form novalidate><h2>Send an enquiry</h2><div class="form-grid"><div class="form-field"><label for="contact-name">Full name <span aria-hidden="true">*</span></label><input id="contact-name" name="name" required /></div><div class="form-field"><label for="contact-phone">Phone <span aria-hidden="true">*</span></label><input id="contact-phone" name="phone" type="tel" required /></div><div class="form-field"><label for="contact-email">Email</label><input id="contact-email" name="email" type="email" /></div><div class="form-field"><label for="contact-type">I am interested in</label><select id="contact-type" name="requirement"><option>Buying a property</option><option>Selling a property</option><option>Rental</option><option>Architecture</option><option>Construction</option><option>Interior Design</option></select></div><div class="form-field field-full"><label for="contact-message">How can we help?</label><textarea id="contact-message" name="notes" rows="5"></textarea></div></div><button class="btn btn-primary" type="submit">Send enquiry</button><p>This form saves a demo lead to this browser only.</p></form></section>`;
}
